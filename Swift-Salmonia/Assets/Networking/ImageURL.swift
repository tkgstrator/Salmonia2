...
class ImageURL {
    ...
    class func weapon(_ weapon_id: Int) -> String {
        ...
        if weapon_id < 0 {
            return "https://app.splatoon2.nintendo.net/images/coop_weapons/" + (weapons.filter({ $0.id == weapon_id }).first?.url ?? "32d41a5d14de756c3e5a1ee97a9bd8fcb9e69bf5.png")
        }
        return "https://app.splatoon2.nintendo.net/images/weapon/" + (weapons.filter({ $0.id == weapon_id }).first?.url ?? "32d41a5d14de756c3e5a1ee97a9bd8fcb9e69bf5.png")
    }
    
    // スペシャル画像のURLを返す関数
    class func special(_ special_id: Int) -> String {
        return "https://app.splatoon2.nintendo.net/images/special/" + (specials.filter({ $0.id == special_id }).first?.url ?? "18990f646c551ee77c5b283ec814e371f692a553.png")
    }
