    ...
    var body: some View {
        GeometryReader { geometry in
            ScrollView(showsIndicators: false) {
                VStack(spacing: 5) {
                    ResultOverview(result: self.$result)
                    HStack(alignment: .top, spacing: 5) {
                        ForEach(Range(1...self.waves.count)) { idx in
                            VStack(spacing: 0) {
                                ResultWaveView(wave: self.$waves[idx - 1])
                                SpecialUseView(special: self.special[idx - 1])
                            }
                        }
                    }
                    VStack {
                        ForEach(Range(1...self.players.count)) { idx in
...
//                             content: {$0.image.resizable().clipShape(RoundedRectangle(cornerRadius: 8.0))}
//                    ).frame(width: 30, height: 30).onTapGesture {
//                        self.takeScreenshot(origin: geometry.frame(in: .global).origin, size: geometry.size)
//                    }
                }
            )
        }
...
private struct ResultOverview: View {
    ...
    var body: some View {
        ZStack {
            URLImage(URL(string: ImageURL.stage(self.result.stage_id.value ?? 5000))!,
                     content: {$0.image.resizable().aspectRatio(contentMode: .fill).clipShape(RoundedRectangle(cornerRadius: 8.0))}
            ).onTapGesture {
                // print(geometry.frame(in: .global).origin, geometry.size)
            ...
            VStack(alignment: .leading, spacing: 0) {
                ZStack {
                    HStack {
                        ZStack(alignment: .leading) {
                            ...
                            Text(Unixtime(interval: result.play_time))
                        ...
                        //                            .padding(.trailing, -60)
                        //                            .padding(.top, -60)
                        //                        Text("\(result.failure_reason == nil ? "Clear!" : "Defeat")")
                        //                            .font(.custom("Splatfont", size: 20))
                        //                        .padding(.trailing, 5)
                        //                        .padding(.top, -30)
                        //
                ...
                HStack {
                    ...
                    Text("x\(self.result.golden_eggs)")
    ...
    var body: some View {
        VStack(spacing: 0) {
            ForEach(usage.indices, id:\.self) { column in
                HStack(spacing: 0) {
                    ForEach(self.usage[column].indices, id:\.self) { idx in
                        URLImage(URL(string: ImageURL.special(self.usage[column][idx]))!, content: {$0.image.resizable()})
        ...
        NavigationLink(destination: SalmonStatsView(nsaid: $player.nsaid)) {
            VStack(spacing: 0) {
                HStack {
                    ...
                    Spacer()
                    URLImage(URL(string: ImageURL.special(player.special_id))!, content: {$0.image.resizable()})
                        .frame(width: 30, height: 30)
                    ForEach(player.weapon_list, id:\.self) { weapon in
                        URLImage(URL(string: ImageURL.weapon(weapon))!, content: {$0.image.resizable()})
                            .frame(width: 30, height: 30)
                    }
                }.frame(height: 30)
                HStack {
